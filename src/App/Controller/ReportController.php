...
class ReportController extends AppController
{
    ...
    public function read($id)
    {
        ...
        foreach ($eventEntries as $key => $event) {
            ...
            $dataAttendance = $modelAttendance->find(array('event' => $event['$id']))->toArray();

            foreach ($dataAttendance as $att => $attendance) {
                $user = $modelUser->findOne($attendance['user']);
                $dataAttendance[$att]['user'] = $user['first_name'];
            }

            $event['attendance'] = $dataAttendance;
